"""K230 offline steel-ball detector for the dataset_70816 recall KModel.

Put this file at /sdcard/main.py on the TF card. CanMV runs it
automatically when powered on, no computer or CanMV IDE needed.

TF-card layout:
  /sdcard/main.py
  /sdcard/models/steel_ball_dataset_70816_yolo26n_416_stretched_recall_i16w8.kmodel

This is the dataset_70816 entry (KALMAN-V2) merged into the shared
runtime script, plus offline additions:
  - display_mode="virt": no LCD/HDMI needed, runs headless
  - the pipeline restarts automatically after 5 s if it crashes
  - the CanMV IDE stop command exits the auto-run loop
...
UART1 (GPIO9=TX, GPIO10=RX) 115200 8N1. Every frame sends one 8-byte
binary frame:
  AA 55 Xh Xl Yh Yl 0D 0A          target present, X/Y = camera pixels
  AA 55 FF FE FF FE 0D 0A          no stable target (lost marker)
"""
...
SCRIPT_VERSION = "STEEL-BALL-DATASET-70816-STRETCHED-416-I16W8-KALMAN-V2-OFFLINE"
KMODEL_PATH = "/sdcard/models/steel_ball_dataset_70816_yolo26n_416_stretched_recall_i16w8.kmodel"
MODEL_INPUT_SIZE = [416, 416]
AI_CAPTURE_SIZE = [512, 288]
DISPLAY_MODE = "virt"
DISPLAY_SIZE = [800, 480]
CONFIDENCE_THRESHOLD = 0.20
DISPLAY_CONFIDENCE_THRESHOLD = 0.20
...
def run_forever():
    """Auto-run loop: restart 5 s after any crash, exit on IDE stop."""
    print("AUTORUN STARTED, restart delay %ds" % RESTART_DELAY_SECONDS)
    while True:
        try:
            main()
        except KeyboardInterrupt:
            print("AUTORUN STOPPED BY IDE")
            break
        except BaseException as exc:
            print("AUTORUN RESTART IN %ds: %s" % (RESTART_DELAY_SECONDS, exc))
